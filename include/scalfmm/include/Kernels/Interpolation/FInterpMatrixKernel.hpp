...
/// One over (a+r) 
template <class FReal>
struct FInterpMatrixKernelAPLUSR : FInterpAbstractMatrixKernel<FReal>
{
    static const KERNEL_FUNCTION_TYPE Type = NON_HOMOGENEOUS;
    static const unsigned int NCMP = 1; //< number of components
    static const unsigned int NPV  = 1; //< dim of physical values
    ...
    FInterpMatrixKernelAPLUSR(const FReal inCoreWidth = 0.01)
    : CoreWidth(inCoreWidth)
    {}

    // copy ctor
    FInterpMatrixKernelAPLUSR(const FInterpMatrixKernelAPLUSR& other)
    : CoreWidth(other.CoreWidth)
    {}

    static const char* getID() { return "ONE_OVER_A_PLUS_R"; }
                        ...
                        const ValueClass& xs, const ValueClass& ys, const ValueClass& zs) const
    {
        ...
        const ValueClass diffy = (yt-ys);
        const ValueClass diffz = (zt-zs);
        const ValueClass r2 = FMath::Sqrt(diffx*diffx+diffy*diffy+diffz*diffz); // 1 / sqrt(r^2)
        return ValueClass(1) / (r2 + ValueClass(CoreWidth));
    }
                                    ...
                                    ValueClass block[1], ValueClass blockDerivative[3]) const
    {
        ...
        const ValueClass diffy = (yt-ys);
        const ValueClass diffz = (zt-zs);
        const ValueClass r_a = FMath::Sqrt(diffx*diffx+diffy*diffy+diffz*diffz);
        const ValueClass one_over_a_plus_r = ValueClass(1) / (r_a + ValueClass(CoreWidth));
        const ValueClass one_over_a_plus_r_squared = one_over_a_plus_r*one_over_a_plus_r;
        const ValueClass one_over_r_a = ValueClass(1) / r_a;

        block[0] = one_over_a_plus_r;

  
        const ValueClass coef = one_over_a_plus_r_squared * one_over_r_a;
        blockDerivative[0] = - coef * diffx;
        blockDerivative[1] = - coef * diffy;
        blockDerivative[2] = - coef * diffz;
